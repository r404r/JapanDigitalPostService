...
7. 除 README 多语版本外，文档正文默认使用中文，同一 Markdown 文件中不得混入日文叙述；只有画面上实际显示的标题、按钮名、表单标签、错误提示等日文 UI 文案可作为专属表述保留，并用引号或代码格式标明。
8. README 为多语例外：`README.md` 是默认日文版，`README.zh-CN.md` 是中文版，`README.en.md` 是英文版。后续功能、部署、使用方式或对外行为变更时，文档影响判定必须判断 README 是否受影响；需要更新时必须同步维护日文、中文、英文三版内容与顶部语言链接。
